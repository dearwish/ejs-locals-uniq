...
Block.prototype = {
	...
	append: function (more, unique) {
		if (!unique || this.html.indexOf(more) === -1) {
			this.html.push(more);
		}
	},
	prepend: function (more, unique) {
		if (!unique || this.html.indexOf(more) === -1) {
 ...
 * @api private
 */
function block (name, html, unique) {
	...
	if (html) {
		blk.append(html, unique);
	}
	return blk;
}

// bound to scripts Block in renderFile
function script (path, type, unique) {
	if (typeof type === "boolean") {
		unique = type;
		type = undefined;
	}
	if (path) {
		this.append('<script src="' + path + '"' + (type ? 'type="' + type + '"' : '') + '></script>', unique);
	}
	return this;
}

// bound to stylesheets Block in renderFile
function stylesheet (path, media, unique) {
	if (typeof media === "boolean") {
		unique = media;
		media = undefined;
	}
	if (path) {
		this.append('<link rel="stylesheet" href="' + path + '"' + (media ? 'media="' + media + '"' : '') + ' />', unique);
	}
	return this;
}
